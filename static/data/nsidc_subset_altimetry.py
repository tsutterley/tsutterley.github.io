...
def nsidc_subset_altimetry(
    ...
    if LATITUDE and LONGITUDE:
        ll = ",".join(
            ["{0:f},{1:f}".format(ln, lt) for ln, lt in zip(LONGITUDE, LATITUDE)]
        )
    ...
    if PLOT:
        ...
        sc = ax1.scatter(
            dinput["X"], dinput["Y"], c=dinput["Z"], s=1, cmap=plt.cm.Spectral_r
        )
        ...
        cbar.ax.set_ylabel("Height Above Reference Ellipsoid", labelpad=10, fontsize=12)
        cbar.ax.set_xlabel("m", fontsize=12)
        cbar.ax.xaxis.set_label_coords(0.5, 1.045)
        # ticks lines all the way across
        cbar.ax.tick_params(
            which="both", width=1, direction="in", length=13, labelsize=12
        )
