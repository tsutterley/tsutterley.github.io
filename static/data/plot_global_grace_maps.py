...
def read_grace_harmonics(base_dir, parameters):
    ...
    return gravtk.grace_input_months(
        base_dir,
        PROC,
        DREL,
        DSET,
...
def plot_grid(base_dir, parameters):
    ...
    if parameters["MEAN_FILE"].title() == "None":
        ...
        mean_Ylms = gravtk.harmonics().from_file(
            parameters["MEAN_FILE"], format=parameters["MEANFORM"], date=False
        )
    ...
    for t, grace_month in enumerate(grace_Ylms.month):
        ...
        data = gravtk.harmonic_summation(
            Ylms.clm, Ylms.slm, glon, glat, LMIN=LMIN, LMAX=LMAX, MMAX=MMAX, PLM=PLM
        ).T
        # set image
        im.set_data(data)
        # add date label (year-calendar month e.g. 2002-01)
        ...
        plt.savefig(
            os.path.join(DIRECTORY, FIGURE_FILE), dpi=figure_dpi, format=figure_format
        )
