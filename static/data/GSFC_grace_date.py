...
def GSFC_mascon_list(VERSION):
    ...
    HOST["v02.4"] = [
        ...
        "grace",
        "mascons_2.4",
        "GSFC.glb.200301_201607_v02.4.hdf",
    ]
    HOST["rl06v1.0"] = [
        "https://earth.gsfc.nasa.gov",
        "sites",
        ...
        "files",
        "2022-01",
        "GSFC.glb_.200204_202110_RL06v1.0_OBP-ICE6GD_0.h5",
    ]
...
def from_http(HOST, timeout=None, local=None, verbose=False, mode=0o775):
    ...
    """
    # get GSFC GRACE mascon file
    req = requests.get(posixpath.join(*HOST), timeout=timeout, allow_redirects=True)
    # get last modified time of GRACE mascon file
    last_modified = req.headers["last-modified"]
    mtime = gravtk.utilities.get_unix_time(
        last_modified, format="%a, %d %b %Y %H:%M:%S %Z"
    )
...
def GSFC_grace_date(input_file, VERSION="v02.4", MODE=0o775):
    ...
    start_yr, M1, D1, h1, m1, s1 = gravtk.time.convert_julian(
        2400000.5 + MJD["ref_days_first"], FORMAT="tuple"
    )
    end_yr, M2, D2, h2, m2, s2 = gravtk.time.convert_julian(
        2400000.5 + MJD["ref_days_last"], FORMAT="tuple"
    )
    YY, MM, DD, hh, mm, ss = gravtk.time.convert_julian(
        2400000.5 + MJD["ref_days_middle"], FORMAT="tuple"
    )
    ...
    dpm_leap = np.array(
        [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], dtype=np.float64
    )
    dpm_stnd = np.array(
    ...
    for t, mon in enumerate(grace_month):
        ...
        print(
            ("{0:13.8f} {1:03d} {2:8.0f} {3:03.0f} {4:8.0f} {5:03.0f} {6:8.0f}").format(
                tdec[t],
                mon,
                start_yr[t],
                start_day[t],
