...
def grace_months(base_dir, DREL=["RL06", "rl06v1.0"]):
    ...
    for rl in DREL:
        ...
        for pr in PROC:
            ...
            if os.access(os.path.join(grace_dir, grace_date_file), os.F_OK):
                ...
                date_input = np.loadtxt(
                    os.path.join(grace_dir, grace_date_file), skiprows=1
                )
                # number of months
    ...
    print(
        '\t<link rel="stylesheet" href="../assets/css/font-awesome.min.css">', file=fid
    )
    print(
        '\t<link rel="stylesheet" href="../assets/css/academicons.min.css">', file=fid
    )
    ...
    # print(
    #     '\t\t\t<a href="../references/presentations.html">Presentations</a>', file=fid
    # )
    # print('\t\t\t<a href="../references/datasets.html">Datasets</a>', file=fid)
    # print(
    #     '\t\t\t<a href="../references/documentation.html">Documentation</a>', file=fid
    # )
    print(
        '\t\t\t<a href="../references/Sutterley_Tyler.pdf">Curriculum Vitae</a>',
        file=fid,
    )
    print('\t\t\t<a href="../news/index.html">News</a>', file=fid)
    print('\t\t\t<a href="../resources/index.html">Resources</a>', file=fid)
    print(
        '\t\t\t<a href="../animations/greenland.html">GRACE Greenland Animation</a>',
        file=fid,
    )
    print(
        '\t\t\t<a href="../animations/antarctica.html">GRACE Antarctic Animation</a>',
        file=fid,
    )
    ...
    for m in range(4, max_mon + 1):
        ...
        print('\t\t\t<td style="text-align:center">{0:03d}</td>'.format(m), file=fid)
        print(
            '\t\t\t<td style="text-align:center">{0}{1:4d}</td>'.format(
                month_string, calendar_year
            ),
            file=fid,
        )
            ...
            if exists != 0:
                ...
                print(
                    (
                        ...
                        "{0:4d}_{1:03d}&ndash;{2:4d}_{3:03d}"
                    ).format(*args),
                    file=fid,
                )
                ...
                print(
                    '\t\t\t\t\t<img class="lfs" data-path="images/{0}">'.format(src),
                    file=fid,
                )
                print("\t\t\t\t</span>", file=fid)
                print("\t\t\t</td>", file=fid)
    ...
    print(
        "\t\t<p><em>GRACE/GRACE-FO anomalies for harmonic solutions are calculated in reference to the 2003"
        "&#8211;2010 mean\n\t\tand are smoothed using a 350km radius Gaussian filter",
        file=fid,
    )
    print(
        (
            """\t\t<a href="#Wahr:1998hy" onmouseover="HighlightRow('Wahr:1998hy')"\n"""
    ...
    print(
        (
            """\t\t<a href="#Swenson:2006hu" onmouseover="HighlightRow('Swenson:2006hu')"\n"""
            """\t\t\tonmouseout="UnhighlightRow('Swenson:2006hu')">"""
            "(Swenson&nbsp;and&nbsp;Wahr,&nbsp;2006)</a>."
        ),
        file=fid,
    )
    ...
    if "RL05" in DREL:
        print(
            (
                "\t\tGRACE Release-5 data products are corrected for pole tide "
                "drift following "
            ),
            file=fid,
        )
        print(
            (
                """\t\t<a href="#Wahr:2015dg" onmouseover="HighlightRow('Wahr:2015dg')"\n"""
    ...
    print(
        (
            "\t\tGSFC GRACE/GRACE-FO mascon data products are calculated as described in "
        ),
        file=fid,
    )
    print(
        (
            """\t\t<a href="#Loomis:2019ef" onmouseover="HighlightRow('Loomis:2019ef')"\n"""
    ...
    print(
        "\t\tGRACE/GRACE-FO fields have been corrected for Glacial Isostatic "
        "Adjustment (GIA) using coefficients from ICE6G Version-D",
        file=fid,
    )
    print(
        (
            """\t\t<a href="#Peltier:2018dp" onmouseover="HighlightRow('Peltier:2018dp')"\n"""
            """\t\t\tonmouseout="UnhighlightRow('Peltier:2018dp')">"""
            "(Peltier&nbsp;et&nbsp;al.&nbsp;,&nbsp;2018)</a>."
        ),
        file=fid,
    )
    ...
    print(
        "\t\t\t\tPost-processing removal of correlated errors in GRACE data.", file=fid
    )
    print("\t\t\t\t<em>Geophysical Research Letters</em>, 33(8), 2006.", file=fid)
    print(
        '\t\t\t\t[&nbsp;<a href="../references/Swenson-2006hu.bib">bib</a>&nbsp;|',
        file=fid,
    )
    print(
        '\t\t\t\t<a href="https://doi.org/10.1029/2005GL025285">http</a>&nbsp;]',
        file=fid,
    )
    print("\t\t\t\t</td>", file=fid)
    print("\t\t\t</tr>", file=fid)
    ...
    print(
        '\t\t\t\t[&nbsp;<a href="../references/Wahr-1998hy.bib">bib</a>&nbsp;|',
        file=fid,
    )
    print(
        '\t\t\t\t<a href="https://doi.org/10.1029/98JB02844">http</a>&nbsp;]', file=fid
    )
    print("\t\t\t\t</td>", file=fid)
    print("\t\t\t</tr>", file=fid)
    ...
    if "RL05" in DREL:
        ...
        print('\t\t\t\t<td class="ref">', file=fid)
        print(
            "\t\t\t\tJ.&nbsp;Wahr, R.&nbsp;S.&nbsp;Nerem and S.&nbsp;V.&nbsp;Bettadpur.",
            file=fid,
        )
        print(
            "\t\t\t\tThe pole tide and its effect on GRACE time-variable gravity measurements:",
            file=fid,
        )
        print(
            "\t\t\t\tImplications for estimates of surface mass variations.", file=fid
        )
        print(
            "\t\t\t\t<em>Journal of Geophysical Research: Solid Earth</em>,", file=fid
        )
        print("\t\t\t\t120(6):4597&#8211;4615, 2015.", file=fid)
        print(
            '\t\t\t\t[&nbsp;<a href="../references/Wahr-2015dg.bib">bib</a>&nbsp;|',
            file=fid,
        )
        print(
            '\t\t\t\t<a href="https://doi.org/10.1002/2015JB011986">http</a>&nbsp;]',
            file=fid,
        )
        print("\t\t\t\t</td>", file=fid)
        print("\t\t\t</tr>", file=fid)
    ...
    print(
        "\t\t\t\tRegularization and error characterization of GRACE mascons.", file=fid
    )
    print("\t\t\t\t<em>Journal of Geodesy</em>,", file=fid)
    print("\t\t\t\t93(9):1381&#8211;1398, 2019.", file=fid)
    print(
        '\t\t\t\t[&nbsp;<a href="../references/Loomis-2019ef.bib">bib</a>&nbsp;|',
        file=fid,
    )
    print(
        '\t\t\t\t<a href="https://doi.org/10.1007/s00190-019-01252-y">http</a>&nbsp;]',
        file=fid,
    )
    ...
    print(
        '\t\t\t\t[&nbsp;<a href="../references/Peltier-2018dp.bib">bib</a>&nbsp;|',
        file=fid,
    )
    print(
        '\t\t\t\t<a href="https://doi.org/10.1002/2016JB013844">http</a>&nbsp;]',
        file=fid,
    )
    print("\t\t\t\t</td>", file=fid)
    print("\t\t\t</tr>\n\t\t</table>\n\t\t</p>\n\t\t</div>", file=fid)
    ...
    print(
        '\t\t<em><a href="./GRACE_months.txt">Table as plain text</a></em></p>',
        file=fid,
    )
    ...
    print(
        (
            ...
            'src="../assets/js/highlight.row.js"></script>'
        ),
        file=fid,
    )
    print(
    ...
    print(
        (
            '\t\t<script type="text/javascript" '
            'src="../assets/js/scale.fix.js"></script>'
        ),
        file=fid,
    )
    print(
        (
            '\t\t<script type="text/javascript" '
            'src="../assets/js/lfs.media.js"></script>'
        ),
        file=fid,
    )
    # print HTML footers
