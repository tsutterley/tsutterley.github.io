...
def compile_regex_pattern(PROC, DREL, DSET, solution='BA01', version='0'):
    #-- GSM RL06: only monthly degree 60 products
    release, = re.findall(r'\d+', DREL)
    args = (DSET, r'(GRAC|GRFO)', r'(UTCSR|GFZOP|JPLEM)', solution,
        release.zfill(2), version.zfill(2))
    regex_pattern = r'{0}-2_\d+-\d+_{1}_{2}_{3}_{4}{5}(\.gz)?$' .format(*args)
    ...
    for pr in PROC:
        ...
        for rl in DREL:
            ...
            for i,mi in enumerate(['grace','grace-fo']):
                ...
                rx = compile_regex_pattern(pr, rl, DSET, version=VERSION[i])
