...
def podaac_grace_sync(
    ...
    if LOG:
        ...
        logging.basicConfig(
            filename=os.path.join(DIRECTORY, LOGFILE), level=logging.INFO
        )
    ...
    files, mtimes = gravtk.utilities.drive_list(
...
def multiprocess_sync(
    remote_file, remote_mtime, local_file, TIMEOUT=0, RETRY=5, MODE=0o775
):
    try:
        output = http_pull_file(
            remote_file,
